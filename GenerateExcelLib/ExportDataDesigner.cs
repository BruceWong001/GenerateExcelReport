...
namespace GenerateExcelLib
{
    ...
    public class ExportDataDesigner<T> : IDisposable
    {
        ...
        private Dictionary<string, IEnumerable<PropertyInfo>> properties_cache = new Dictionary<string, IEnumerable<PropertyInfo>>();
        ...
        private T Data;
        public Dictionary<string, Tuple<int, int, int, int>> MergeCells
        {
            get
            {
                return m_MergeCells;
        ...
        private int DrillDown(Object _data, int currentCol, int currentRowIndex = 0, Boolean needAddCol = true, DataRow reuse_Row = null)
        {
            ...
            var newTypeFullName = newType.FullName;
            if(!properties_cache.ContainsKey(newTypeFullName))
            {
                var newProperties = newType.GetProperties(BindingFlags.Public | BindingFlags.Instance).ToList();
                properties_cache.Add(newTypeFullName, newProperties);
            }
            foreach (var property_Item in properties_cache[newTypeFullName])
            {
